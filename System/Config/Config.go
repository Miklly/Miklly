...
import (
	...
	. "github.com/miklly/miklly/System/Log"
)
...
func processXmlTocken(c *Config, xmlName, data string) {
	...
	case "WebRoot":
		c.WebRoot = changePathSeparator(data)
		if !strings.HasSuffix(c.WebRoot, "/") {
			c.WebRoot = StrAdd(c.WebRoot, "/")
		}
	case "IndexFiles":
		c.IndexFiles = strings.Split(data, ";")
	case "SessionType":
