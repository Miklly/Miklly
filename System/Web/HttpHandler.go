...
import (
	...
	. "github.com/miklly/miklly/System/Config"
...
func (this *HttpHandler) ProcessStatic(requestPath string, w http.ResponseWriter, r *http.Request) bool {
	...
	//区分大小写比较
	strPath := Function.StrAdd(AppConfig.WebRoot, requestPath)
	if Function.FileExist(strPath) {
		http.ServeFile(w, r, strPath)
		return true
	}
	//转换为小写，不区分大小写的比较
	strLowerPath := strings.ToLower(requestPath)
	strPath = Function.StrAdd(AppConfig.WebRoot, strLowerPath)
	if Function.FileExist(strPath) {
		http.ServeFile(w, r, strPath)
		return true
	}
